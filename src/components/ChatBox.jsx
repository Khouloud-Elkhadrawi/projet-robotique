...
function ChatBox({ onMenuClick, isLoggedIn, onLoginRequest }) {
  ...
  /* Envoi du message — bloqué si non connecté */
  const sendMessage = () => {
  ...
  return (
    <div className="chatbox">
      ...
      {/* ── Zone des messages ── */}
      <section className={`chatbox-messages ${!isLoggedIn ? 'chatbox-messages--locked' : ''}`} aria-label="Messages">
      ...
      <footer className="chatbox-input-area">
        <div className={`input-wrapper ${!isLoggedIn ? 'input-wrapper--locked' : ''}`}
          onClick={!isLoggedIn ? onLoginRequest : undefined}
          title={!isLoggedIn ? 'Connectez-vous pour écrire un message' : ''}
        >
          <textarea
            ref={inputRef}
            className="chatbox-input"
            placeholder={isLoggedIn ? "Posez votre question à RoboChat…" : "Connectez-vous pour écrire un message…"}
            value={inputText}
            onChange={e => isLoggedIn && setInputText(e.target.value)}
          ...
          />
          <button
            className={`send-btn ${isLoggedIn && inputText.trim() ? 'send-btn--active' : ''}`}
            onClick={isLoggedIn ? sendMessage : onLoginRequest}
            disabled={!isLoggedIn || !inputText.trim() || isTyping}
            aria-label="Envoyer"
          >
            {!isLoggedIn ? (
              /* Icône cadenas quand non connecté */
